...
const TokenDisplay = () => {
  ...
  const loadTokens = async () => {
    try {
      ...
      try {
        ...
        firebaseTokens = tokensSnapshot.docs.map((doc) => ({
          id: doc.id,
        ...
        console.warn("Firebase tokens unavailable:", fbErr.message);
      }

      const allTokens = [...localTokens, ...firebaseTokens];
      ...
      const tokensWithData = await Promise.all(
        allTokens.map(async (token) => {
          if (token.isLocal) {
            return {
              ...token,
              price: mockPriceFor(token.id || token.mintAddress),
              metadata: null,
            };
          }
          ...
          try {
            ...
            if (token.mintAddress) {
              try {
                ...
                console.warn(
                  `Price unavailable for ${token.name}:`,
                  priceErr.message
                );
              }
            }
            ...
            if (token.metadataUrl) {
              try {
                ...
                console.warn(
                  `Metadata unavailable for ${token.name}:`,
                  metadataErr.message
                );
              }
            }
            ...
            console.warn(`Error processing ${token.name}:`, err.message);
            return {
              ...token,
              price: "N/A",
            };
  ...
  useEffect(() => {
    ...
    window.addEventListener("localTokens:updated", handleUpdate);
    window.addEventListener("storage", handleUpdate);

    return () => {
      window.removeEventListener("localTokens:updated", handleUpdate);
  ...
  return (
    <div className=" mx-5 px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tokens.map((token) => {
          const displayImage = token.metadata?.image || token.imageUrl;
          const isDataUrl =
            typeof displayImage === "string" && displayImage.startsWith("data:");
                ...
                <p className="text-sm w-fit font-medium py-1 px-2 bg-green-100 text-green-700 border rounded-lg border-green-200">
                  ...
                  {formatMarketCap(
                    (token.price && token.price !== "N/A"
                      ? Number(token.price)
                      : mockPriceFor(token.id || token.mintAddress)) *
                      1_000_000_000
                  )}
                </p>
              </div>
